...
exports.processMaintenanceReminders = exports.sendOwnerDailySummary = exports.rescheduleOverdueAppointments = exports.processReminders = void 0;
const ReminderJob_1 = __importDefault(require("../models/ReminderJob"));
...
const formatSlot = (date) => `${String(date.getHours()).padStart(2, '0')}:${String(date.getMinutes()).padStart(2, '0')}`;
const buildDateTime = (baseDate, slot) => {
...
const processReminders = async () => {
    const now = new Date();
    const jobs = await ReminderJob_1.default.find({
        status: 'PENDING',
        runAt: { $lte: now }
    });
    ...
    for (const job of jobs) {
        try {
            const appointment = await Appointment_1.default.findById(job.appointmentId).populate('clientId');
            if (!appointment) {
...
const mapRequestTypeToLabel = (requestType) => requestType === 'repair' ? 'Reparacion' : 'Diagnostico / Presupuesto';
const rescheduleOverdueAppointments = async () => {
    const now = new Date();
    const startOfToday = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    const targetDate = new Date(startOfToday);
    targetDate.setDate(targetDate.getDate() + 1);
    const overdueAppointments = await Appointment_1.default.find({
    ...
    for (const appointment of overdueAppointments) {
        ...
        const workOrder = await WorkOrder_1.default.findOne({ appointmentId: appointment._id });
        ...
        const durationMs = appointment.endAt && appointment.startAt
...
exports.rescheduleOverdueAppointments = rescheduleOverdueAppointments;
const sendOwnerDailySummary = async () => {
    ...
    const [appointments, pendingRequests] = await Promise.all([
        Appointment_1.default.find({
            ...
            .sort({ startAt: 1 }),
        AppointmentRequest_1.default.find({ status: 'PENDING' }).sort({ createdAt: -1 }).limit(30),
    ]);
    const statusTotals = appointments.reduce((acc, appointment) => {
        if (appointment.status === 'CONFIRMED')
            acc.confirmed += 1;
        if (appointment.status === 'IN_PROGRESS')
            acc.inProgress += 1;
...
exports.sendOwnerDailySummary = sendOwnerDailySummary;
const processMaintenanceReminders = async () => {
    const now = new Date();
    const startOfToday = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()));
    const endOfToday = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), 23, 59, 59, 999));
    const workOrders = await WorkOrder_1.default.find({
    ...
    for (const wo of workOrders) {
      ...
      `;
            await (0, mailer_1.sendEmail)({ to: email, subject, html });
...
exports.processMaintenanceReminders = processMaintenanceReminders;
//# sourceMappingURL=cronProcessor.js.map